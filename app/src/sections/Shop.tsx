            ...
            <div key={product.id} className={`group bg-[#f9f5f0] rounded-3xl overflow-hidden transition-all duration-700 hover:shadow-2xl hover:-translate-y-2 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'}`} style={{ transitionDelay: `${300 + index * 150}ms` }}>
              ...
              <div className="p-6">
                  ...
                  <div className="mb-4">
                    ...
                    <div className="flex gap-2">
                      {product.colors.map((color) => (
                        <button key={color} onClick={() => selectColor(product.id, color)} className={`w-6 h-6 rounded-full ${getColorClass(color)} border-2 transition-all ${(selectedColors[product.id] || product.colors[0]) === color ? 'border-gray-900 scale-110' : 'border-transparent hover:scale-105'}`} title={color} />
                      ))}
                    </div>
                  </div>
                )}
                ...
                <Button asChild className="w-full flex items-center justify-center gap-2 transition-all duration-300 btn-primary">
                  <a href={product.affiliateUrl} target="_blank" rel="sponsored noopener noreferrer" aria-label={`Check ${product.name} options on ${product.retailer}`}>
